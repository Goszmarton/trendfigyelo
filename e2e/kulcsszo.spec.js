...
// ── ADATFORRÁS-MARKER (teljes nézet): kapcsolóval BE/KI (alapból KI), a meres_kezdete-nél jelölő ──
// A heti nyers sorozat (racs_nyersRekord …,52,7) a NAP_MS=2026-08-01 → +364 nap ablakot fed.
// A marker MINDEN charton megjelenik (bekapcsolva), a látható tartományra CSÍPVE ha kívül esik.
function _korhaz_mock(page, mk) {
  return mock(page, {
    // NINCS érvényes órás (1_het hibás) → a teljes = az 1_ev másodlagos (2026-08-01 → 2027-07-31)
    regObj: reg({ "kórház": regSzo({ domen: "egeszseg", meres_kezdete: mk, intervallumok: {
      "1_het": ivHibas("keves_pont"), "2_het": ivHibas("keves_pont"), "1_ho": ivHibas("keves_pont"),
    ...
    mpRegObj: mpReg({ "kórház": mpSzo("het", { "1_het": ivHibas("keves_pont"), "2_het": ivHibas("keves_pont"),
      ...
      { domen: "egeszseg", meres_kezdete: mk }) }),
    mpNyersObj: mpNyers({ "kórház": [racs_nyersRekord("kórház", 52, 7)] }),
  });
}

test("adatforrás-marker: alapból KI (nincs marker), a gomb + info jelen, kattintás bekapcsolja", async ({ page }) => {
  await _korhaz_mock(page, "2026-10-01");   // mk az ablakon belül
  await page.goto("/trendek.html");
  const k = page.locator('#kulcsszo-blokk .kulcsszo-chart[data-kulcsszo="kórház"]');
  await k.waitFor();
  await expect(k).not.toHaveAttribute("data-elo-marker", /.*/);                  // ALAPBÓL KI: nincs marker
  const gomb = page.locator("#kulcsszo-blokk .adatforras-gomb");
  await expect(gomb).toHaveCount(1);                                             // a gomb jelen
  await expect(page.locator("#kulcsszo-blokk .adatforras-info")).toHaveCount(1); // a kék-vonalas info jelen
  await gomb.click();                                                           // BEKAPCSOLÁS
  await expect(page.locator('#kulcsszo-blokk .kulcsszo-chart[data-kulcsszo="kórház"]'))
    .toHaveAttribute("data-elo-marker", "2026-10-01");                          // most megjelenik a marker
});

test("adatforrás-marker: bekapcsolva az ablak ELŐTTI meres_kezdete is markert kap (csípve a szélre)", async ({ page }) => {
  await _korhaz_mock(page, "2026-06-01");   // mk az ablak (2026-08-01) ELŐTT
  await page.goto("/trendek.html");
  await page.locator("#kulcsszo-blokk .adatforras-gomb").click();               // bekapcsol
  await expect(page.locator('#kulcsszo-blokk .kulcsszo-chart[data-kulcsszo="kórház"]'))
    .toHaveAttribute("data-elo-marker", "2026-06-01");                          // MINDEN charton látszik (csípve)
});
...
test("ML-trend: kikapcsolva → nincs lila görbe, nincs nemlineáris-metrika a kártyán", async ({ page }) => {
  ...
  await page.goto("/trendek.html");   // a kapcsoló alapból KI — NEM kattintunk
  const vanLila = await page.evaluate(() => {
    const p = (window.chart_peldanyok || {})["kórház"];
    return !!p && p.data.datasets.some(d => d.borderColor === "#8e44ad");
  });
