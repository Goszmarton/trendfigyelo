...
def _szam_tartomany(ertek, hol: str, minimum: int) -> int:
    """int(ertek), KonfigHiba ha nem egész VAGY < minimum (a numerikus config-kulcsok tartomány-őre).

    A 0/negatív érték EDDIG csendesen átment (`int(...)`) — pl. `trend_idosor_max: 0` a RESZBEN-TELT-BLOKK
    leak-et config-elérhetővé tette. A ≥1 kulcsok legalább egyet követelnek; a ≥0 (ADD-ON) kulcsoknál a 0 legitim.
    """
    try:
        n = int(ertek)
    except (ValueError, TypeError):
        raise KonfigHiba(f"{hol}: egész szám kell, nem {ertek!r}")
    if n < minimum:
        raise KonfigHiba(f"{hol}: legalább {minimum} kell, kaptam {n}-t")
    return n
...
def betolt(utvonal="config.yaml") -> Config:
    ...
    return Config(
        ...
        idoablak_orak=_szam_tartomany(_kell(nyers, "idoablak_orak", ""), "idoablak_orak", 1),
        ...
        trend_idosor_rekesz_max=_szam_tartomany(nyers.get("trend_idosor_rekesz_max", 5), "trend_idosor_rekesz_max", 0),
        tortenet_visszapotlas_nap=_szam_tartomany(nyers.get("tortenet_visszapotlas_nap", 3), "tortenet_visszapotlas_nap", 0),
